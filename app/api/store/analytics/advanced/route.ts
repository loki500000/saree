...
// GET - Get advanced analytics for store using optimized SQL functions
export async function GET(request: NextRequest) {
  try {
    ...
    // Use optimized SQL function for daily analytics
    ...
    if (dailyError) {
      console.error("Fetch daily analytics error:", dailyError);
      return NextResponse.json(
        { error: dailyError.message || "Failed to fetch daily analytics" },
        { status: 500 }
      );
    }
    ...
    // Get top users using SQL function
    ...
    if (topUsersError) {
      console.error("Fetch top users error:", topUsersError);
    }

    // Get popular clothing using SQL function
    ...
    // Get hourly usage pattern using SQL function
    ...
    if (hourlyError) {
      console.error("Fetch hourly pattern error:", hourlyError);
    }
    ...
    if (creditError) {
      console.error("Fetch credit breakdown error:", creditError);
    }
