-- ============================================
-- FIXED STORE COMPARISON ANALYTICS FUNCTIONS
-- This version uses proper type casting to handle VARCHAR columns
...
BEGIN
    ...
    WITH store_tryons AS (
        SELECT
            ...
            MODE() WITHIN GROUP (ORDER BY EXTRACT(HOUR FROM th.created_at)) AS peak_hour
        FROM
            stores s
        LEFT JOIN
    ...
    store_credits AS (
        SELECT
            s.id AS store_id,
            COALESCE(SUM(CASE WHEN ct.type = 'purchase' THEN ct.amount ELSE 0 END), 0) AS total_credits_purchased,
    ...
    SELECT
        ...
        COALESCE(st.total_credits_used, 0) AS total_credits_used,
        COALESCE(sc.total_credits_purchased, 0) AS total_credits_purchased,
        COALESCE(sc.total_credits_refunded, 0) AS total_credits_refunded,
        CASE
            WHEN st.unique_users > 0 THEN ROUND(st.total_tryons::NUMERIC / st.unique_users, 2)
            ELSE 0
        END AS avg_tryons_per_user,
        CASE
            WHEN st.total_tryons > 0 THEN ROUND(st.total_credits_used::NUMERIC / st.total_tryons, 2)
            ELSE 0
        END AS avg_credits_per_tryon,
...
BEGIN
    ...
    ELSIF p_metric = 'credits' THEN
        RETURN QUERY
        SELECT
            ROW_NUMBER() OVER (ORDER BY SUM(th.credits_used) DESC NULLS LAST) AS rank,
            s.id AS store_id,
            s.name::TEXT AS store_name,
            s.slug::TEXT AS store_slug,
        ...
        GROUP BY
            s.id, s.name, s.slug
        ORDER BY
            SUM(th.credits_used) DESC NULLS LAST;
...
DO $$
BEGIN
    RAISE NOTICE '✅ Fixed store analytics functions deployed successfully!';
    RAISE NOTICE 'Type casting has been added to handle VARCHAR columns';
    RAISE NOTICE 'You can now use the store comparison page at /admin/analytics/store-comparison';
END $$;
